...
class SSU(Function):
    @staticmethod
    def forward(ctx, input, alpha, epsilon):
        # t = f(1-x)/f(x)  x>0
...
class gtnet(nn.Module):
    def __init__(self, train_feas, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None,
                 static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32,
        ...
        self.Adjacency_generator = Adjacency_generator(embedding_size=256, num_nodes=num_nodes,
                                                       time_series=self._train_feas,
                                                       kernel_size=12, freq=168,
                                                       requires_graph=1,
                                                       input_dim=1,
                                                       device=device)
        ...
        for i in range(1):
            ...
            for j in range(1, layers + 1):
                ...
                self.V_at.append(Spatial_Attention_layer(device, conv_channels, num_nodes, T_steps[j]))
    ...
    def forward(self, input, idx=None):
        ...
        if self.gcn_true:
            if self.buildA_true:
                if idx is None:
                    adp = SmoothSparseUnit(self.Adjacency_generator(input), 1, 0.10)
                else:
                    adp = SmoothSparseUnit(self.Adjacency_generator(input), 1, 0.10)
            else:
